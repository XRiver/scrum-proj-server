...
CREATE TABLE `apply` (
  ...
  PRIMARY KEY (`applyid`)
) ENGINE=InnoDB AUTO_INCREMENT=37 DEFAULT CHARSET=utf8 COMMENT='申请信息';

/*Data for the table `apply` */

insert  into `apply`(`applyid`,`openid`,`pid`,`mess`,`pass`,`deal`) values 
(36,'o9VBV44gvvycu-kp8dnCoBI8npe8',43,'我想去',1,1);
...
CREATE TABLE `attraction` (
  ...
  PRIMARY KEY (`aId`)
) ENGINE=InnoDB AUTO_INCREMENT=38 DEFAULT CHARSET=utf8 COMMENT='景点信息';
...
(2,'中山陵','南京市玄武区石象路7号','https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=1112168980,2047313080&fm=27&gp=0.jpg','中山陵是我国伟大的革命先行者孙中山先生的陵墓，坐落在紫金山第二峰小茅山的南麓，北依青山，南临平川，依山就势，气势雄伟，使人有仰止崇高之感。孙中山先生1925年逝世于北京，临终时他希望将遗体葬于南京钟山，故死后其灵柩暂厝于北京香山碧云寺内，为了尊重他的遗愿，于翌年3月12日在他逝世一周年之日，在钟山开始建墓，至1929年春落成，同年6月1日就将其灵柩从北京移葬于此。'),
...
(17,'红山动物园','南京下关区','images/attractions/e3dbe741-64f3-4f80-a219-ff94d7f5fed4.jpg','这是一个动物园。');
...
insert  into `evaluation`(`eid`,`pid`,`fromid`,`toid`,`mess`,`star`) values 
(24,43,'o9VBV4yQfCMsqJif_6DdAEhKzAIo','o9VBV44gvvycu-kp8dnCoBI8npe8',NULL,5);
...
CREATE TABLE `plan` (
  ...
  PRIMARY KEY (`pId`)
) ENGINE=InnoDB AUTO_INCREMENT=44 DEFAULT CHARSET=utf8 COMMENT='出行计划信息';

/*Data for the table `plan` */

insert  into `plan`(`pId`,`openId`,`aId`,`travelTime`,`detail`,`applyList`,`state`) values 
(43,'o9VBV4yQfCMsqJif_6DdAEhKzAIo','3','1549296000000','总统府一日游','o9VBV44gvvycu-kp8dnCoBI8npe8,','2');
...
CREATE TABLE `user` (
  ...
  PRIMARY KEY (`uId`)
) ENGINE=InnoDB AUTO_INCREMENT=27 DEFAULT CHARSET=utf8 COMMENT='用户信息';

/*Data for the table `user` */

insert  into `user`(`uId`,`openId`,`uName`,`school`,`pictureUrl`,`sex`,`nickName`,`city`) values 
(25,'o9VBV4yQfCMsqJif_6DdAEhKzAIo','陆天骁','南京大学','https://wx.qlogo.cn/mmopen/vi_32/DYAIOgq83erUVryEBQuUN6hnAhCdqZBvapewsTf9DZ0XibGWsianj7YGoTo9GUUYIoKiaJGclkEteP8bljML8bxicA/132','男',NULL,'Suzhou'),
(26,'o9VBV44gvvycu-kp8dnCoBI8npe8','何炜杰','南大','https://wx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTK9vP0sq9CPps6aQpQ07ZXJFqiaia9icl8khJkGlGoSTR1m2kaFdtGvMj9gNXagxdsaxcxOiaEoaoxSmA/132','男',NULL,'Wuxi');
